...
"""
Description: Script to generate Anki flash card importable/compatible 
TSV files from SQLite database file.

Modified from: https://github.com/eyedol/tools/blob/master/anki_data_builder.py

Sample usage: ./anki_data_builder.py ~/cards-jwasham-extreme.db ./anki-card.tsv
"""
...
class AnkiCardBuilder:
    ...
    def __write_csv(self, items):
        with open(self.csv_output, 'w', newline='', encoding='utf-8') as file:
            csvw = csv.writer(file, delimiter='\t', lineterminator='\n')
            csvw.writerow(['# FRONT', 'BACK'])
            for item in items:
                print(item)
                csvw.writerow([
                  item[0],
